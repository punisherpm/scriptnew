...
<!-- Модалка оформления драфта -->
<div id="draft-modal" style="
  display:none;
  position:fixed;
  inset:0;
...
">
  <div style="background:#fff; padding:20px; border-radius:8px; min-width:320px;">
    <h3>Оформить драфт</h3>
    <p>
      Стартовая цена выкупа:<br>
      <input type="text" id="draft_start_price" readonly style="width:100%;">
    </p>
    <p>
      Шаг ставки (1–5 млн):<br>
      <input type="number" id="draft_step" min="1000000" max="5000000" step="500000" value="1000000" style="width:100%;">
    </p>
    <div style="margin-top:10px; text-align:right;">
      <button type="button" onclick="closeDraftModal()">Отмена</button>
      <button type="button" onclick="submitDraft()">Подтвердить</button>
    </div>
  </div>
</div>
...
function parseValue(valueStr) {
  ...
  let clean = valueStr.replace('€', '').trim().toLowerCase();
  let multiplier = 1;
  let numStr = clean;
...
function extractPlayerValue(html) {
  var doc = document.createElement('div');
  doc.innerHTML = html;
  var cols = doc.querySelectorAll('.grid .col');
  for (var col of cols) {
    var sub = col.querySelector('.sub');
    if (sub && sub.textContent.trim().toLowerCase() === 'value') {
      var em = col.querySelector('em');
      if (em) return parseValue(em.textContent.trim());
    }
  }
  return null;
}
...
function openDraftModal() {
  if (!lastPlayerData || !lastPlayerData.on_transfer) {
    alert('Сначала загрузите игрока и его стоимость.');
    return;
  }
  document.getElementById('draft_start_price').value = lastPlayerData.playerValue;
...
function submitDraft() {
  const step = parseInt(document.getElementById('draft_step').value, 10);
  if (isNaN(step) || step < 1000000 || step > 5000000) {
    alert('Шаг ставки должен быть от 1 до 5 млн (в целых евро).');
    return;
  }
  ...
  .then(data => {
    if (data.success) {
      alert('Драфт оформлен, ID игрока: ' + data.player_id);
      closeDraftModal();
    } else {
...
async function convertSofifa() {
  ...
  try {
    ...
    // читаем как текст и пытаемся распарсить как JSON
    ...
    try {
      ...
      resultEl.textContent = 'AllOrigins вернул не JSON:\n' + text.slice(0, 300);
      return;
    }

    if (!data || typeof data.contents !== 'string') {
      loaderEl.style.display = 'none';
      resultEl.textContent = 'Пустой или странный ответ от AllOrigins';
      return;
    }
...
`);
    ...
    let playerValue = extractPlayerValue(html);

    // лучшая позиция
    let bestPositionWithStar = convertedPositions.find(p => p.endsWith('*')) || convertedPositions[0] || '';
    let positionsAllPlain = convertedPositions
      .map(p => p.replace(/\*$/, ''))
      .join(', ');

    function hasAbility(name) {
      return specialAbilities.includes(name) ? 1 : 0;
    }
    ...
    lastPlayerData = {
      ...
      name: name,
      position: bestPositionWithStar || '',
      positions_all: positionsAllPlain,
      salary: 0,
      club_id: 0,
      is_on_transfer: 1,
      ...
      height: jsonLd.height || null,
      weight: jsonLd.weight || null,
      foot_side: side,
      weak_foot_acc: Number.isFinite(weakFootAcc) ? weakFootAcc : null,
      weak_foot_freq: Number.isFinite(weakFootFreq) ? weakFootFreq : null,
      consistency: Number.isFinite(consistency) ? consistency : null,
      condition: Number.isFinite(condition) ? condition : null,
      injury: injuryTolerance || 'B',
      needs_salary_update: 0,
      ...
      response: pesResp,
      agility: pesAgi,
      dribble_accuracy: pesDribAcc,
      dribble_speed: pesDribSpeed,
      short_pass_accuracy: pesSPA,
      short_pass_speed: pesSPS,
      long_pass_accuracy: pesLPA,
      long_pass_speed: pesLPS,
      shot_accuracy: pesShotAcc,
      shot_power: pesShotPw,
      shot_technique: pesShotTech,
      free_kick: pesFK,
      curling: pesCurling,
      header: pesHeading,
      jump: pesJump,
      ...
      dribbling:          hasAbility('Dribbling'),
      tactical_dribble:   hasAbility('Tactical dribble'),
      positioning:        hasAbility('Positioning'),
      reaction_sa:        hasAbility('Reaction'),
      playmaking:         hasAbility('Playmaking'),
      passing:            hasAbility('Passing'),
      scoring:            hasAbility('Scoring'),
      one_on_one:         hasAbility('1-on-1 Scoring'),
      post_player:        hasAbility('Post player'),
      lines:              hasAbility('Lines'),
      middle_shooting:    hasAbility('Middle shooting'),
      side_sa:            hasAbility('Side'),
      centre:             hasAbility('Centre'),
      penalties_sa:       hasAbility('Penalties'),
      one_touch_pass:     hasAbility('1-Touch Pass'),
      outside:            hasAbility('Outside'),
      marking:            hasAbility('Marking'),
      sliding:            hasAbility('Sliding'),
      covering:           hasAbility('Covering'),
      d_line_control:     hasAbility('D-Line control'),
      penalty_stopper:    hasAbility('Penalty stopper'),
      one_on_one_stopper: hasAbility('1-on-1 stopper'),
      long_throw:         hasAbility('Long Throw')
    };
...
<div class="player-container">
  <div class="player-left">
    ...
    <div class="player-info liquid-glass">
      ...
      <p><strong>Стоимость:</strong> ${playerValue !== null ? playerValue.toLocaleString('ru-RU') + ' €' : 'N/A'}</p>
      <p><strong>Позиции:</strong> ${positionsHtml}</p>
      <p><button type="button" onclick="openDraftModal()">Оформить драфт</button></p>
    </div>
